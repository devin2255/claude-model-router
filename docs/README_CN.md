...
提示：`model-router model` 会列出配置文件中的 models 并提供交互式选择。

## 配置方式

建议先运行 `model-router init` 生成
`model-router.config.json`，再填写密钥。若未安装 Python，
`model-router init` 会尝试通过 `winget` 自动安装，安装后
...
- `model-router model` 会列出配置文件中的 models 并提供交互式选择；回车则自动检测公网 IP 并选择默认模型。
- `model-router <模型名>` 切换后会输出更新后的配置详情。
- `model-router openai` 会检测 IP，如在中国大陆会提示可能不可用，并询问是否继续或改用 kimi。
- `model-router claude` 会输出当前配置详情，并在当前终端启动 Claude CLI。
- `mr` 为 `model-router` 的短命令别名。
- `/model` 或 `-m` 仍可用（兼容旧用法）。
- 以管理员身份运行可写入系统环境变量；非管理员可能写入失败且仅对当前进程生效。
- `model-router openai` 会自动启动本地协议转换代理，并将 `ANTHROPIC_BASE_URL` 指向代理地址。
...
现在 `model-router openai` 已内置并自动启动协议转换代理，代理地址会写入
